import { MATERIAL_COSTS, DEFAULT_SETTINGS } from '../types';
import { apiService } from './apiService';
export class FinancialCalculator {
    static async generateReport(period, startDate, endDate) {
    ...
    static calculateEmployeeSalary(employee, bagsSold, period) {
        let total = 0;
    ...
    static calculateMaterialCostPerBag() {
        return {
     ...
     */
    static async calculateCommissionFromSales(employeeId, startDate, endDate) {
        ...
        const employeeSales = safeAllSales.filter(sale => sale.employeeId !== undefined &&
     ...
     */
    static async calculateCommissionFromPackerEntries(employeeId, startDate, endDate) {
        ...
        // Filter by employeeId - must match exactly and not be undefined/null
        const employeeEntries = safeAllEntries.filter(entry => entry.employeeId !== undefined &&
