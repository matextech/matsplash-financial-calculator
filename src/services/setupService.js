 ...
 */
export async function initializeDefaultDirector() {
    try {
        const users = await apiService.getUsers();
        // Handle both array and object with data property
        const usersList = Array.isArray(users) ? users : (users.data || []);
        // Check if any director exists
        const directorExists = usersList.some((u) => u.role === 'director');
        if (!directorExists) {
...
export async function initializeDefaultAccounts() {
    try {
        ...
        // Receptionist
        const receptionist = usersList.find((u) => u.role === 'receptionist');
        if (!receptionist) {
            await apiService.createUser({
                name: 'Receptionist',
        ...
        // Storekeeper
        const storekeeper = usersList.find((u) => u.role === 'storekeeper');
        if (!storekeeper) {
            await apiService.createUser({
                name: 'Storekeeper',
